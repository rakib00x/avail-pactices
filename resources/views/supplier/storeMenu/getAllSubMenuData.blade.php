                ...
                <tr>
                    ...
                    <td>
                        <div class="custom-control custom-switch custom-control-inline mb-1">
                            <input type="checkbox" class="custom-control-input changeSubMenuStatus" <?php if($value->status == 1){ echo 'checked'; }else{ echo ''; } ?> getSubMenuid="{{$value->id}}" id="customSwitch{{$value->id}}">
                            <label class="custom-control-label mr-1" for="customSwitch{{$value->id}}"></label>
                        </div>
                    </td>
                    <td>
                        <div class="invoice-action">
                            <a onclick="editsub('{{$value->id}}')" data-toggle="modal" id="edit" data-target="#editsubModal" href="" class="invoice-action-edit cursor-pointer">
                                <i style="font-size:25px;" class="bx bx-edit"></i>
                            </a>
                            <a onclick="deleteSubMenu('{{$value->id}}')" class="invoice-action-view mr-1" style="cursor: pointer;">
